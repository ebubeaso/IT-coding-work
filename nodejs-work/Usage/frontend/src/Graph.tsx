...
// set up the Graph function

// margin type
type Margin = {"top": number, "left": number, "bottom": number, "right": number}
const margins: Margin = {"top": 20, "left": 20, "bottom": 20, "right": 20}
// make the function component
export const XboxGraph: React.FC = () => {
    ...
    useEffect(() => {
        fetch("http://192.168.1.103:9900/usage").then(response => {return response.json()})
                .then(data => {
                    ...
                    const graphWidth: number = width - margins.left - margins.right;
                    const graphHeight: number = height - margins.top - margins.bottom;
                    ...
                    var lineGenerator = d3.line<any>()
                            .x(function(d:{day:string, hours:number}): any {
                                return xScale(d.day);
                            })
                            .y(function(d:{day:string, hours:number}): any {
                    ...
                    svg.selectAll("circle").data(result)
                            ...
                            .attr("cx", (d:{day:string, hours:number}): any => {
                                return xScale(d.day);
                            })
                            .attr("cy", (d:{day:string, hours:number}): any => {
                                return yScale(d.hours);
                            }).attr("transform", "translate(" + (margins.left*2).toString()+ ", 0)");
            });
        }, [])
    return (
        <div>
        <h1 className="Title">Hours Spent on the Xbox</h1>
        <p id="xbox"></p>
        </div>
    )
}
...
export const Phone: React.FC = () => {
    useEffect(() => {
        fetch("http://192.168.1.103:9900/phone").then(response => {return response.json()})
            ...
            let canvas = d3.select("svg");
                ...
                var phoneWidth: number = parseInt(canvas.attr("width"));
                var phoneHeight: number = parseInt(canvas.attr("height"));
                var chartWidth: number = phoneWidth - margins.left - margins.right;
                var chartHeight: number = phoneHeight - margins.top - margins.bottom;
                ...
                var lineGen = d3.line<any>().x( (d:{day:string, hours: number}): any => {
                    return scaleX(d.day);
                }).y( (d:{day: string, hours: number}): any => {
    ...
    return (
        <div>
            <h1 className="Title">Hours Spent on the Phone</h1>
            <p id="phone"></p>
        </div>
    )
}
...
export const Main: React.FC = () => {
    ...
    return (
        <div>
        <h1 className="Title">Hello there sport!</h1>
        <p style={Style}>This is the main page</p>
        </div>
    )
}
